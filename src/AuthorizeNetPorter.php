 ...
 */
class AuthorizeNetPorter extends
    OmnipayPorterAbstract
{
    ...
    public function newGatewayDataEntity(string $endpoint_id): object
    {
        ...
        // Return the gateway
        return new AuthorizeNetGatewayData(
            $api,
     ...
     * @throws \Exception
     */
    public function newAuthorizeNetAuthorizeRequestController(
    ): AuthorizeNetAuthorizeRequestController
    {

        // Create the AuthorizeNetQueryRequestData entity
        $request_data = new AuthorizeNetAuthorizeRequestData(
            'authorize',
            $this->getOffsiteURLs()
        );

        // Return a new AuthorizeNetQueryRequestController entity
        return new AuthorizeNetAuthorizeRequestController(
     ...
     * @throws \Exception
     */
    public function newAuthorizeNetCaptureRequestController(
    ): AuthorizeNetCaptureRequestController
    {

        // Create the AuthorizeNetQueryRequestData entity
        $request_data = new AuthorizeNetCaptureRequestData(
            'capture',
            $this->getOffsiteURLs()
        );

        // Return a new AuthorizeNetQueryRequestController entity
        return new AuthorizeNetCaptureRequestController(
            $this->getSavedGatewayDataEntity('default'),
            $request_data,
